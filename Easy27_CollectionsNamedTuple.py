"""
Basically, namedtuples are easy to create, lightweight object types.
They turn tuples into convenient containers for simple tasks.
With namedtuples, you don’t have to use integer indices for accessing members of a tuple.

Referecne: https://www.pythontutorial.net/advanced-python/python-namedtuple/

Example:

from collections import namedtuple

#Point is the name of a the class
# namedtuple is the imported module, but also functions as a superclass
# x and y become field names
Point = namedtuple('Point','x,y')

#pt1 and pt2 become objects of type Point
pt1 = Point(1,2)
pt2 = Point(3,4)

#Now we call values by their field names within the objects
dot_product = ( pt1.x * pt2.x ) +( pt1.y * pt2.y )
print dot_product

"""

from collections import namedtuple
n = int(input()) # Number of Students
a = input()
# a takes in the field names, which become the keys
# a also sets the order of the field
total = 0 # creates the value for totaling the marks
Student = namedtuple('Student', a)
# namedtuple serves as method, and sort of a superclass
# Student is the actual class

for _ in range(n):
    student = Student(*input().split())
    # student is the object of type Student
    total += int(student.MARKS)
print('{:.2f}'.format(total/n))